package com.mesosphere.sdk.dremio.scheduler;
...
public class ServiceTest {
    ...
    public void testSpec() throws Exception {
        ...
        ticks.add(Send.offerBuilder("dremio").build());
        ticks.add(Expect.launchedTasks("dremio-0-node"));
        ticks.add(Send.taskStatus("dremio-0-node", Protos.TaskState.TASK_ERROR).build());
        ...
        ticks.add(Send.offerBuilder("dremio").setPodIndexToReoffer(0).build());
        ticks.add(Expect.launchedTasks("dremio-0-node"));
        ticks.add(Send.taskStatus("dremio-0-node", Protos.TaskState.TASK_RUNNING).build());
